    ...
    def forward(self, hidden_states, input_ids, attention_mask=None, target_token_id=10004, eval=False):
        ...
        for i in range(batch_size):
            ...
            if len(target_positions) == 0:
                # eval=True used to fall through to target_positions[0] unguarded, so a
                # generation that did not emit the action marker raised IndexError and
                # killed the whole run. per_sample.csv is written only when the eval
                # finishes, so one such record at index 500/600 destroys every result.
                # Fall through to the flag=False path and let the caller decide.
                # (Training is unaffected: len == 0 already hit `continue` below.)
                continue
...
class ChameleonXLLMXForConditionalGeneration_ck_action_head(ChameleonForConditionalGeneration):
    ...
    def generate_action_head(self, input_ids, generation_config):
        """Continuous decoder: emit ONE token (expected 10004), then read the
        regression head off the prompt's hidden states.

        Returns a (time_horizon, action_dim) tensor in the SAME normalised
        bin-centre space generate_dis_ma returns, so the caller applies the
        identical unnorm_waypoints(); or None when the head could not be applied.

        NEVER returns zeros. A zero trajectory is a plausible-looking "stay at the
        origin" plan that scores worse than the model-free prior, so a silent zero
        is read as a RESULT ("the continuous head is bad") rather than as a
        failure. Callers that want a zero fallback must write it themselves.
        """
        self.init_input_ids = None

        # exactly one token. The head consumes the PROMPT's hidden states, so every
        # further autoregressive step is a wasted forward -- and callers hand us the
        # config sized for the discrete decoder (~32 new tokens).
        cfg = copy.deepcopy(generation_config)
        cfg.max_new_tokens = 1
        # set max_length consistently too, or HF warns and the precedence between
        # the two becomes version-dependent
        cfg.max_length = int(input_ids.shape[1]) + 1

        res = ChameleonForConditionalGeneration.generate(
            self, input_ids=input_ids, generation_config=cfg,
        ...
        # 检查是否成功预测动作
        if not actions_flag:
            # the model did not emit token 10004, so the head was applied to a
            # placeholder row -- the numbers it returned mean nothing
            return None

        expected = self.action_head.time_horizon * self.action_head.action_dim
        if predicted_actions.numel() != expected:
            # a head configured for a different horizon than the caller's; reshaping
            # would silently reinterpret the values
            return None

        # 将predicted_actions重新reshape为[time_horizon, action_dim]
        return predicted_actions.reshape(self.action_head.time_horizon, self.action_head.action_dim)
