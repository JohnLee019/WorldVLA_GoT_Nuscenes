...
class LiberoFinetuneConversation(Dataset):
    def __init__(self, config_path, resolution, with_state=True, with_wrist=True, with_action=True, with_world_model=True):
        ...
        # Imported lazily so that merely importing this module (which the
        # pretrain solver does at top level) does not require the LIBERO /
        # robosuite / mujoco stack. Non-LIBERO tasks (e.g. nuScenes driving)
        # never construct this class and so never need the simulator.
