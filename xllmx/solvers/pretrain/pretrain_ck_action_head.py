...
class PretrainSolverBase_ck_action_head(ABC):
    ...
    def get_args_parser(cls):
        ...
        parser.add_argument(
            ...
            default="adamw",
            help="optimizer: 'adamw' (default fp32 AdamW). 'adamw8bit'/'paged_adamw8bit' use "
            "bitsandbytes 8-bit states to cut optimizer memory ~4x, enabling --trainable full 7B "
            "on 24GB cards (e.g. 3x4090). 'paged_adamw8bit' additionally pages states to CPU on "
            "OOM spikes. Requires `pip install bitsandbytes`.",
        )
    ...
    def build_model(self) -> (nn.Module, Tokenizer):
        ...
        if hasattr(unwrapped_model, "get_trainable_params"):
            ...
            for key, param in unwrapped_model.named_parameters():
                if key in trainable_params or 'lora' in key:
                    # fp32 master copy: optimizer.step() accumulates updates on the
                    # order of lr*grad (~1e-7), which bf16's 8-bit mantissa would
                    # round straight back to the old value.
                    param.requires_grad = True
                    promote_param_to_fp32(param)
                else:
                    # Frozen params never reach optimizer.step(), so an fp32 master
                    # buys nothing -- FSDP's MixedPrecision re-casts params to
                    # mixed_precision_dtype on every all-gather regardless, making
                    # bf16 -> fp32 -> bf16 a round trip to the same values at twice
                    # the memory (~13GB on a 7B backbone). Hold them at compute dtype.
        ...
        if opt_name in ("adamw8bit", "paged_adamw8bit"):
            try:
                ...
                raise ImportError(
                    f"--optimizer {opt_name} requires bitsandbytes; run `pip install bitsandbytes`"
                ) from e
            opt_cls = bnb.optim.PagedAdamW8bit if opt_name == "paged_adamw8bit" else bnb.optim.AdamW8bit
            opt = opt_cls(model.parameters(), lr=self.args.lr, weight_decay=self.args.wd, betas=(0.9, 0.95))
            self.logger.info(f"using bitsandbytes {opt_name} optimizer (8-bit states)")
        else:
