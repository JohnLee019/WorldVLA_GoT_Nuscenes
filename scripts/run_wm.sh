...
torchrun --nproc_per_node=3 train_nuscenes_wm.py \
  ...
  --data_config_val_ood ./data/nuscenes_wm_records/nuscenes_wm_v1.0-trainval_val.json \
  --output_dir ./output/nuscenes_wm_trainval_full \
  --trainable full --optimizer paged_adamw8bit \
  --batch_size 1 --accum_iter 8 --resolution 256 --grad_precision bf16 \
  --save_iteration_interval 2000 --ckpt_max_keep 4 \
  --epochs 5 --lr 1e-4 --precision bf16 --checkpointing
